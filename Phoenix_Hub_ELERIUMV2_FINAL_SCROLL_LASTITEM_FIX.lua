	...
	do -- UI Elements
		...
		do -- Add Tab
			function window_data:AddTab(tab_name)
				...
				-- Calculate the REAL bottom of the last rendered control.
				-- UIListLayout.AbsoluteContentSize can lag behind on mobile, which
				-- previously left the final controls outside the scrollable canvas.
				local function updateTabCanvas()
					if not new_tab or not new_tab.Parent then return end
					local bottom = 0
					for _, child in ipairs(new_tab:GetChildren()) do
						if child:IsA("GuiObject") and child ~= new_tab_layout then
							local relBottom = (child.AbsolutePosition.Y - new_tab.AbsolutePosition.Y) + child.AbsoluteSize.Y
							if relBottom > bottom then bottom = relBottom end
						end
					end
					if new_tab_layout then
						bottom = math.max(bottom, new_tab_layout.AbsoluteContentSize.Y)
					end
					-- Extra bottom padding makes the final item fully reachable on touch.
					local contentHeight = bottom + 40
					new_tab.CanvasSize = UDim2.new(0, 0, 0, math.max(contentHeight, new_tab.AbsoluteSize.Y + 1))
				end
				if new_tab_layout then
				...
				-- Recalculate after layout/render passes so the last control is never clipped.
				RS.Heartbeat:Connect(updateTabCanvas)
				task.defer(updateTabCanvas)
